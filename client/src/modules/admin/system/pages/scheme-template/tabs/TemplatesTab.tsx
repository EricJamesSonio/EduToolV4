...
import { PROGRAM_TYPES, getProgramTypeLabel } from '@/modules/admin/academic/constants/programTypes';
import {
  COMPONENT_TYPES,
...
const ComponentRow: React.FC<ComponentRowProps> = ({
  row, index, isFirst, canRemove, onChange, onRemove,
}) => (
...
const TemplateFormPanel: React.FC<TemplateFormPanelProps> = ({
  form, isSubmitting, error, isEdit, onFormChange, onSubmit, onCancel,
}) => {
  ...
  const handleRemove = useCallback((index: number) => {
    onFormChange({ ...form, components: form.components.filter((_, i) => i !== index) });
  }, [form, onFormChange]);
  ...
  return (
    <div className="grading-scheme-form-panel">
      ...
      {error && (
        <p className="system-section-note" style={{ color: 'var(--color-error-600)', marginBottom: 'var(--spacing-4, 1rem)' }}>
          {error}
        </p>
      )}
...
const TemplateCard: React.FC<TemplateCardProps> = ({
  template, isEditing, isDeleting, editForm, editError, isSubmitting,
  onEdit, onEditFormChange, onEditSubmit, onEditCancel,
      ...
      <div className="grading-scheme-card-header">
        <div className="grading-scheme-card-meta">
          ...
          <span className="grading-scheme-card-type">
            {template.programType ? getProgramTypeLabel(template.programType) : 'General'}
          </span>
        </div>
        {!isEditing && !isDeleting && (
          <div className="grading-scheme-card-actions">
            <button type="button" className="btn btn-sm btn-outline" onClick={() => onEdit(template)}>
              Edit
            </button>
            <button type="button" className="btn btn-sm btn-outline" onClick={() => onDeleteRequest(template.id)}>
              Delete
            </button>
          </div>
        )}
      ...
      {isDeleting && (
        <div className="grading-scheme-delete-confirm">
          ...
          <div className="grading-scheme-delete-actions">
            <button type="button" className="btn btn-sm btn-outline" onClick={onDeleteCancel}>
              Cancel
            </button>
            <button
              type="button"
...
const TemplatesTab: React.FC = () => {
  ...
  return (
    <div className="grading-scheme-templates-tab">
      <div className="grading-scheme-tab-header">
        {!showCreateForm && (
