// src/components/educator/presentation-builder/PreviewModal.tsx
"use client";

import { ChevronLeft, ChevronRight } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { SlideDraft, getFontStack } from "./types";
...
export function PreviewModal({
  slides, templateImage, currentSlide, onClose, onNavigate,
}: PreviewModalProps) {
  ...
  const fontStack = getFontStack(slide.fontFamily);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center">
      ...
      >
        ...
        {/* Slide content — font family applied here */}
        <div
          className="relative z-[5] h-full flex flex-col items-center justify-center p-8 md:p-16 text-center text-white drop-shadow-lg"
        ...
        <div className="absolute bottom-0 inset-x-0 h-12 bg-zinc-900/60 flex items-center justify-between px-4 z-10">
          <Button
            variant="ghost" size="sm" className="text-white/80 hover:text-white"
          ...
          <Button
            variant="ghost" size="sm" className="text-white/80 hover:text-white"
            onClick={() => onNavigate(Math.min(slides.length - 1, currentSlide + 1))}
