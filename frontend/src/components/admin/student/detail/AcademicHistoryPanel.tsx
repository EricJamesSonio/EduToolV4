...
import { useAcademicFullHistory, useAcademicTimeline } from "@/hooks/admin/useAcademicHistory";
import { AcademicHistoryDetailsView } from "./AcademicHistoryDetailsView";
import { EmptyState } from "@/components/shared/EmptyState";
import { History } from "lucide-react";
import { GraduationCap, Users, BookOpen, Award, ArrowRightLeft, FileText } from "lucide-react";
...
function renderDetails(type: string, data: Record<string, unknown>): React.ReactNode {
  ...
  switch (type) {
    ...
    case "outcome_set": {
      ...
      return <span className="flex items-center gap-2"><Badge variant={variant as never} className="capitalize text-xs">{outcome}</Badge><span className="text-sm">{d.subjectName as string}</span>{d.outcomeReason ? <span className="text-xs text-muted-foreground">· {String(d.outcomeReason)}</span> : null}</span>;
    }
    case "program_shift":
      return <span className="text-sm">{d.fromProgramName as string} <span className="text-muted-foreground mx-1">→</span> {d.toProgramName as string}<Badge variant="outline" className="ml-2 text-xs capitalize">{String(d.defaultOutcomeUsed ?? "").replace(/_/g, " ")}</Badge></span>;
...
export function AcademicHistoryPanel({ studentId, schoolYearId }: Props): React.JSX.Element {
  const [view, setView] = useState<"details" | "timeline">("details");

  const { data: history, isLoading: historyLoading } = useAcademicFullHistory(studentId);
  const { data: timeline, isLoading: timelineLoading } = useAcademicTimeline(studentId, schoolYearId, "asc");

  const isLoading = view === "details" ? historyLoading : timelineLoading;

  const filteredHistory = (() => {
    if (!history || !schoolYearId) return history ?? [];
    return (history as unknown as { schoolYear: { id: string } }[]).filter((h) => h.schoolYear.id === schoolYearId) as unknown as typeof history;
  })();
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">{view === "details" ? "Current enrollment & classes" : `${timeline?.length ?? 0} milestone${(timeline?.length ?? 0) !== 1 ? "s" : ""} · With dates`}</p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <AcademicHistoryDetailsView history={(filteredHistory as never) ?? []} />
      ) : (timeline ?? []).length === 0 ? (
        <EmptyState title="No timeline events" description={schoolYearId ? "No milestones for this school year yet." : "No history yet."} icon={History} />
      ) : (
