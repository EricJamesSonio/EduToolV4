...
import { studentApi, type BulkCreateStudentResult, type BulkCreateResponse } from "@/api/admin/student.api";
...
export function BulkCreateStudentDialog({
  ...
  const handleDownload = () => {
    if (!results) return;
    downloadCsv(
      results.created.map((r: BulkCreateStudentResult) => ({
        "Full Name": r.fullName,
        Email: r.email,
        "Student ID": r.studentId,
        Password: r.plainPassword,
      ...
      {!results ? (
        ...
        <div className="space-y-4">
          ...
          <div className="max-h-[360px] overflow-y-auto space-y-2 rounded-lg border bg-muted/40 p-3">
            {results.created.map((r: BulkCreateStudentResult, i: number) => (
              <div key={i}>
