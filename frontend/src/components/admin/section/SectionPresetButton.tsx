...
import type { SectionPreset, SectionPresetData } from "@/hooks/admin/useSectionPreset";
...
interface SectionPresetButtonProps {
  schoolYearId: string;
  programs: Program[];
  preset: SectionPreset | null;
  savePreset: (data: SectionPresetData) => void;
  setEnabled: (enabled: boolean) => void;
  clearPreset: () => void;
}

export function SectionPresetButton({
  schoolYearId,
  programs,
  preset,
  savePreset,
  setEnabled,
  clearPreset,
}: SectionPresetButtonProps): React.JSX.Element {
  ...
  // Departments without course/strand groups get their levels straight from
  // the program — matching how SectionDialog filters levels.
  const { data: programLevels = [] } = useAsyncQuery(
    ...
    { enabled: !!programId && !hasCourses && !hasStrands },
  );
    ...
    <>
      ...
      {open && (
        <Modal
          ...
          title="Section Preset"
          description="Pick a department, course/strand, and level. New Section will auto-fill with these until you turn the preset off or change it."
          size="md"
        >
          <ModalBody>
                ...
                <div className="flex items-center justify-between rounded-lg border bg-muted/40 px-3 py-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {preset.enabled
                        ? "New Section auto-fills with this preset."
                        : "Preset is off — New Section opens blank."}
                    </p>
                  </div>
                  <Button
                    type="button"
