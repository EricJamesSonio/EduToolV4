  ...
  return (
    <div className="rounded-lg border bg-card overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b bg-muted/30">
        ...
        <span className="text-sm font-semibold not-interactive">Strands</span>
        <Badge variant="secondary" className="text-xs font-normal">
      ...
      <div className="divide-y">
        {strands.length === 0 ? (
          <p className="px-4 py-6 text-sm text-muted-foreground not-interactive">No strands found.</p>
        ) : strands.map((strand) => (
          <CountRow
            key={strand.id}
            label={strand.name}
