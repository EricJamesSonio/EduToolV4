...
function GroupBlock({
  ...
  const { data: levels = [], isLoading } = useQuery<Level[]>({
    queryKey: ["admin", "levels", schoolYearId, groupType, groupId],
    queryFn:  () =>
      groupType === "course"
        ? levelApi.getByCourse(schoolYearId, groupId)
        : levelApi.getByStrand(schoolYearId, groupId),
  });
...
export function CourseGroupBlock({
  course,
  ...props
}: { course: CourseSnapshot } & LevelListSharedProps): React.JSX.Element {
  const label = course.code ? `${course.code} – ${course.name}` : course.name;
  return <GroupBlock label={label} groupId={course.id} groupType="course" {...props} />;
}

export function StrandGroupBlock({
  strand,
  ...props
}: { strand: StrandSnapshot } & LevelListSharedProps): React.JSX.Element {
  return <GroupBlock label={strand.name} groupId={strand.id} groupType="strand" {...props} />;
}