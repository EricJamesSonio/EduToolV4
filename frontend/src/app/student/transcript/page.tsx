// ===== File: frontend/src/app/student/transcript/page.tsx =====
"use client";
...
import type {
  ...
  TranscriptSemester,
  TranscriptClass,
} from "@/api/student/transcript.api";
...
function gradeStatusBadge(
  ...
  return (
    <Badge
      variant="outline"
      className="text-[11px] font-semibold text-foreground border-border/80"
    >
      {finalGrade}
    </Badge>
  );
...
function schoolYearStatusBadge(status: string) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn(
        "text-[11px] font-medium capitalize shrink-0",
        cls
      )}
    >
      {status}
...
function SubjectRows({ cls }: { cls: TranscriptClass }) {
  const sorted = [...cls.termGrades].sort(
...
function SchoolYearAccordion({
  year,
}: {
  year: TranscriptYear;
}) {
  const [open, setOpen] = useState(
    year.schoolYearStatus === "active"
  );
