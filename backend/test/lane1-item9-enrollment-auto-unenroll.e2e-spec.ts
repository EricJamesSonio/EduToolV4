...
runSuite('Lane 1 item 9 — auto-unenroll when the school year ends (real DB)', () => {
  ...
  const orgA = { id: `e2e9a-org-${uuid()}`, slug: `e2e9a-${uuid().slice(0, 8)}` };
  const orgB = { id: `e2e9b-org-${uuid()}`, slug: `e2e9b-${uuid().slice(0, 8)}` };
  const orgC = { id: `e2e9c-org-${uuid()}`, slug: `e2e9c-${uuid().slice(0, 8)}` };
  ...
  async function orgWithExpiredYear(
    ...
    await db.organization.create({ data: { id: org.id, name, slug: org.slug } });
  ...
  async function classFixture(orgId: string, syId: string) {
    const program = await db.program.create({
      data: { org_id: orgId, school_year_id: syId, name: 'JHS Item9', type: 'jhs' },
    });
    const level = await db.level.create({
      data: { org_id: orgId, school_year_id: syId, program_id: program.id, name: 'Grade 7' },
    });
  ...
  async function activeClassEnrollment(
    ...
    const ssy = await db.studentSchoolYear.create({
      data: { org_id: orgId, student_id: student.id, school_year_id: syId, status: 'active' },
    });
    const enrollment = await db.enrollment.create({
      data: { org_id: orgId, class_id: classId, student_id: student.id, status: 'active' },
    });
    return { studentId: student.id, ssyId: ssy.id, enrollmentId: enrollment.id };
  }
    ...
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        // on the year-end path, so the remaining deps can stay unneeded.
        {
          ...
          useFactory: (db_: DatabaseService, orgSetting: OrgEnrollmentSettingService) =>
            new SchedulerTasks(null as never, null as never, null as never, orgSetting, null as never, db_, null as never),
  ...
  it('(a) cron unenrolls class enrollments + students for an expired active year', async () => {
    return check('item9 (a) scheduler auto-unenroll on year end', async () => {
      const sy = await orgWithExpiredYear(orgA, 'Item9a Org');
      const cls = await classFixture(orgA.id, sy.id);
      const enr = await activeClassEnrollment(orgA.id, sy.id, cls.classId, 'a');

      await scheduler.handleAutoUnenrollOnYearEnd();

      const yearAfter = await db.schoolYear.findUnique({ where: { id: sy.id } });
      expect(yearAfter?.status).toBe('ended');

      const enrollmentAfter = await db.enrollment.findUnique({ where: { id: enr.enrollmentId } });
      expect(enrollmentAfter?.status).toBe('removed');

      const ssyAfter = await db.studentSchoolYear.findUnique({ where: { id: enr.ssyId } });
  ...
  it('(b) org setting auto_unenroll_on_year_end=false preserves enrollments (year still ends)', async () => {
    return check('item9 (b) disabled auto-unenroll setting respected', async () => {
      const sy = await orgWithExpiredYear(orgB, 'Item9b Org');
      const cls = await classFixture(orgB.id, sy.id);
      const enr = await activeClassEnrollment(orgB.id, sy.id, cls.classId, 'b');
      await orgSettingService.upsert(orgB.id, { auto_unenroll_on_year_end: false });
  ...
  it('(c) SchoolYearService.end() scrubs enrollments unconditionally and its guards hold', async () => {
    return check('item9 (c) admin end() scrub + guards', async () => {
      ...
      const enr = await activeClassEnrollment(orgC.id, sy.id, cls.classId, 'c');
      ...
      await expect(schoolYearService.end(pendingSy.id, orgC.id, actorId)).rejects.toBeInstanceOf(BadRequestException);
      await expect(schoolYearService.end(sy.id, orgC.id, actorId)).rejects.toBeInstanceOf(ConflictException);
      await expect(
        schoolYearService.end('00000000-0000-4000-8000-000000000000', orgC.id, actorId),
