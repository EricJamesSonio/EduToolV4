...
async function check(label: string, fn: () => void | Promise<unknown>): Promise<void> {
  try {
    await fn();
    log(`PASS ${label}`);
  } catch (err) {
...
function signToken(secret: string | undefined, sub: string): string {
  if (!secret) throw new Error('JWT_SECRET missing from env — cannot sign token.');
...
describe('[late-enrollment] grading exclusion — real DB end to end', () => {
  ...
  let b1Id = '';
  let p1Id = '';
  let tenantCleanups: Array<(d: DatabaseService) => Promise<unknown>> = [];
  let ownerToken = '';
  let intruderToken = '';
  ...
  beforeAll(async () => {
    ...
    orgId = org.id;
    tenantCleanups.push((d) => d.organization.delete({ where: { id: org.id } }));
    ...
    tenantCleanups.push((d) => d.schoolYear.delete({ where: { id: schoolYear.id } }));

    const program = await db.program.create({
      data: { org_id: org.id, school_year_id: schoolYear.id, name: 'P1', type: 'college' },
    });
    ...
    const gradingScale = await db.gradingScale.create({
      data: {
        ...
        ranges: [
          { minPercent: 0, maxPercent: 74, gradeValue: 'F', remark: 'Failed', isPassing: false },
          { minPercent: 75, maxPercent: 100, gradeValue: 'P', remark: 'Passed', isPassing: true },
        ],
      },
    });
    tenantCleanups.push((d) => d.gradingScale.delete({ where: { id: gradingScale.id } }));
    ...
    tenantCleanups.push((d) => d.gradingScaleAssignment.deleteMany({ where: { org_id: org.id } }));

    const mkAccount = async (role: string) => {
      const a = await db.account.create({
        data: { org_id: org.id, role: role as any, email: `${uuid().slice(0, 12)}@late.test`, password: 'x', status: 'active' },
      });
    ...
    const semester = await db.semester.create({
      data: { org_id: org.id, school_year_id: schoolYear.id, name: 'Sem 1', start_date: new Date(now.getFullYear(), 0, 1), end_date: new Date(now.getFullYear() + 1, 0, 1) },
    });
    tenantCleanups.push((d) => d.semester.delete({ where: { id: semester.id } }));

    const term = await db.term.create({
      data: { org_id: org.id, semester_id: semester.id, name: 'Term 1', order_index: 1, start_date: new Date(now.getFullYear(), 0, 1), end_date: new Date(now.getFullYear() + 1, 0, 1) },
    });
    termId = term.id;
    ...
    await db.enrollment.create({
      data: { org_id: org.id, class_id: classMixId, student_id: studentId, status: 'active' },
    });
    // All-pre class enrollment.
    await db.enrollment.create({
      data: { org_id: org.id, class_id: classAllPreId, student_id: studentId, status: 'active' },
    });
    tenantCleanups.push((d) => d.enrollment.deleteMany({ where: { org_id: org.id } }));

    const scheme = await db.gradingScheme.create({
      data: { org_id: org.id, class_id: classMixId, name: 'E2E Scheme', is_default: true },
    });
    tenantCleanups.push((d) => d.gradingScheme.delete({ where: { id: scheme.id } }));
    const comps = [
    ...
    for (const c of comps) {
      ...
      tenantCleanups.push((d) => d.gradingSchemeComponent.delete({ where: { id: cc.id } }));
    }

    const mkAssessment = async (classId: string, type: string, title: string, release: Date, totalItems = 50) => {
    ...
    tenantCleanups.push((d) => d.grade.deleteMany({ where: { org_id: org.id } }));
    tenantCleanups.push((d) => d.assessmentGradingOverride.deleteMany({ where: { org_id: org.id } }));
    tenantCleanups.push((d) => d.auditLog.deleteMany({ where: { org_id: org.id } }));
...
const breakdownByCategory = async (classId: string) => {
  const termGrades = await svc.getGradesByTerm(classId, termId, orgId, ownerId);
  const student = termGrades.students.find((s: any) => s.studentId === studentId);
    ...
    return {
      ...
      performance: student.categoryBreakdown.find((c: any) => c.type === 'performance'),
    };
  };
  ...
  it('(2) mixed: only post-enrollment assessments are averaged; full drop renormalizes', async () => {
    ...
    await check('scenario 2 — final grade = 80 (performance category dropped, not zero-penalized)', async () => {
      const grade = await computeAndFetchGrade(classMixId);
      expect(grade?.final_score).toBe(80);
    });

    await check('scenario 2 — excluded cells carry reason default_excluded', async () => {
      const termGrades = await svc.getGradesByTerm(classMixId, termId, orgId, ownerId);
      const student = termGrades.students.find((s: any) => s.studentId === studentId);
      const b1 = student.assessmentScores.find((s: any) => s.assessmentId === b1Id);
  ...
  it('(6) HTTP: override POST works for the owner; non-owner is rejected over real HTTP', async () => {
    ...
    await check('HTTP — unauthenticated status call rejects with 401', async () => {
      await http
        .get(`/classes/${classMixId}/grades/students/${studentId}/assessments/status`)
        .expect(401);
    });
    ...
    await check('HTTP — non-owner POST override rejects with 403', async () => {
      await http
        .post(`/classes/${classMixId}/grades/students/${studentId}/assessments/${b1Id}/override`)
        .set('Authorization', `Bearer ${intruderToken}`)
        .send({ overrideStatus: 'MISSING' })
        .expect(403);
    });

    await check('HTTP — owner POST override succeeds and is audited', async () => {
      await http
        .post(`/classes/${classMixId}/grades/students/${studentId}/assessments/${b1Id}/override`)
      ...
      const res = await http
        ...
        .set('Authorization', `Bearer ${ownerToken}`)
        .expect(200);
      const statusList = Array.isArray(res.body) ? res.body : (res.body as any)?.data;
    ...
    await check('HTTP — owner DELETE override reverts and removes the row', async () => {
      await http
        .delete(`/classes/${classMixId}/grades/students/${studentId}/assessments/${b1Id}/override`)
        .set('Authorization', `Bearer ${ownerToken}`)
        .expect(200);
    ...
    await check('HTTP — non-owner DELETE override rejects with 403', async () => {
      await http
        .delete(`/classes/${classMixId}/grades/students/${studentId}/assessments/${b1Id}/override`)
        .set('Authorization', `Bearer ${intruderToken}`)
        .expect(403);
  ...
  it('(3) override include: all three assessments average and grade changes exactly', async () => {
    if (!available) return;
    await check('scenario 3 — write include override for the pre-enrollment B1', async () => {
      await svc.setAssessmentStatusOverride(classMixId, b1Id, studentId, orgId, ownerId, {
        overrideStatus: 'MISSING',
      } as any);
    });

    await check('scenario 3 — written rawAverage = (40+70+90)/3 = 66.67', async () => {
      const bd = await breakdownByCategory(classMixId);
      expect(bd.written.rawAverage).toBe(66.67);
    });

    await check('scenario 3 — final grade = 66.67 (all three counted)', async () => {
      const grade = await computeAndFetchGrade(classMixId);
  ...
  it('(4) override then delete: grade reverts exactly to the default-exclusion result', async () => {
    if (!available) return;
    await check('scenario 4 — delete the override via the service (Phase 3 endpoint path)', async () => {
      const res = await svc.deleteAssessmentStatusOverride(classMixId, b1Id, studentId, orgId, ownerId);
  ...
  it('(5) exempted submission stays exempt even when override would include the assessment', async () => {
    ...
    await check('scenario 5 — default exclusion still yields 80 (exemption is independent)', async () => {
      const grade = await computeAndFetchGrade(classMixId);
      expect(grade?.final_score).toBe(80);
    });

    await check('scenario 5 — forced include does NOT resurrect an exempted submission', async () => {
      await svc.setAssessmentStatusOverride(classMixId, b1Id, studentId, orgId, ownerId, {
        overrideStatus: 'MISSING',
      } as any);
      const grade = await computeAndFetchGrade(classMixId);
      ...
      await svc.deleteAssessmentStatusOverride(classMixId, b1Id, studentId, orgId, ownerId);
    });
  });

  it('(1) all-pre-enrollment component reports no eligible assessments, not a 0%', async () => {
    if (!available) return;
    await check('scenario 1 — categoryBreakdown is all-exempted, not 0', async () => {
      const termGrades = await svc.getGradesByTerm(classAllPreId, termId, orgId, ownerId);
      const student = termGrades.students.find((s: any) => s.studentId === studentId);
      const written = student.categoryBreakdown.find((c: any) => c.type === 'written');
      expect(written.rawAverage).toBeNull();
      expect(written.isAllExempted).toBe(true);
    });

    await check('scenario 1 — final score is 0 with no active categories (non-div-by-zero)', async () => {
