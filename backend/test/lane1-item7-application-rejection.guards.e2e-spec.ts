...
} from '@/modules/enrollment-portal/dto/enrollment-portal.dto';
...
runSuite('Lane 1 item 7 — rejection flow, data-trail guard, write-time state machine (real DB)', () => {
  let app: INestApplication;
  let db: DatabaseService;
  let portal: EnrollmentPortalService;
  let registrar: EnrollmentRegistrarService;

  const org = { id: `e2e7-org-${uuid()}`, slug: `e2e7-${uuid().slice(0, 8)}` };
  ...
  beforeAll(async () => {
    ...
    await db.organization.create({ data: { id: org.id, name: 'E2E Item 7 Org', slug: org.slug } });
    ...
    program = await db.program.create({
      data: { org_id: org.id, school_year_id: sy.id, name: 'Elementary E2E 7', type: 'elementary' },
    });
    level = await db.level.create({
      data: { org_id: org.id, school_year_id: sy.id, program_id: program.id, name: 'Grade 1' },
    });
  ...
  it('(a) rejected application is committed to the data trail and can never be silently re-placed', async () => {
    return check('item7 (a) rejection data trail + no re-placement', async () => {
      ...
      const created = await portal.createApplication(org.slug, period.token, session, dtoFor('Amy Rejected'));
      log(`(a) created application ${created.application_code} -> ${created.status}`);
      expect(created.status).toBe('pending');

      // Registrar rejects it with a reason.
      const rejected = await registrar.rejectApplication(org.id, actorId, created.id, {
      ...
      const row = await db.enrollmentApplication.findUnique({ where: { id: created.id } });
      expect(row?.status).toBe('rejected');
      expect(row?.reviewed_by).toBe(actorId);
      expect(row?.rejection_reason).toBe('Incomplete documents — missing birth certificate.');
      ...
      expect(metadata?.reason).toBe('Incomplete documents — missing birth certificate.');
      expect(metadata?.personal_email).toBe(email);

      // The SAME applicant tries to re-apply in the same school year.
      await expect(
        portal.createApplication(org.slug, period.token, session, dtoFor('Amy Rejected')),
  ...
  it('(b) the write-time guard is state-based: rejected stays editable, but the review path refuses to approve it', async () => {
    return check('item7 (b) rejected application state machine', async () => {
      ...
      const created = await portal.createApplication(org.slug, period.token, session, dtoFor('Robert Rejected'));
      await registrar.rejectApplication(org.id, actorId, created.id, { reason: 'Duplicate submission.' });
      ...
      const editSession: EnrollmentSessionClaims = { ...session, applicationId: created.id };
      const updated = await portal.updateApplication(org.slug, period.token, editSession, {
  ...
  afterAll(async () => {
    ...
    await step('applications', () => db.enrollmentApplication.deleteMany({ where: { org_id: org.id } }));
    await step('student enrollments', () => db.studentProgramEnrollment.deleteMany({ where: { org_id: org.id } }));
    await step('student school-year', () => db.studentSchoolYear.deleteMany({ where: { org_id: org.id } }));
    await step('accounts', () => db.account.deleteMany({ where: { org_id: org.id } }));
    await step('audit logs', () => db.auditLog.deleteMany({ where: { org_id: org.id } }));
    await step('periods', () => db.enrollmentPeriod.deleteMany({ where: { org_id: org.id } }));
    await step('sections', () => db.section.deleteMany({ where: { org_id: org.id } }));
    await step('levels', () => db.level.deleteMany({ where: { org_id: org.id } }));
    await step('programs', () => db.program.deleteMany({ where: { org_id: org.id } }));
    await step('school years', () => db.schoolYear.deleteMany({ where: { org_id: org.id } }));
    await step('organization', () => db.organization.deleteMany({ where: { id: org.id } }));
    await app.close();
  }, 120000);
});