...
runSuite('Original Lane 1 item 6 — enrollment approval race + capacity oversubscription (real DB)', () => {
  ...
  const orgA = { id: `e2e6a-org-${uuid()}`, slug: `e2e6a-${uuid().slice(0, 8)}` };
  const orgB = { id: `e2e6b-org-${uuid()}`, slug: `e2e6b-${uuid().slice(0, 8)}` };
  ...
  async function programLevelFixture(orgId: string, schoolYearId: string) {
    const program = await db.program.create({
      data: { org_id: orgId, school_year_id: schoolYearId, name: 'Elementary E2E', type: 'elementary' },
    });
    const level = await db.level.create({
      data: { org_id: orgId, school_year_id: schoolYearId, program_id: program.id, name: 'Grade 1' },
    });
  ...
  async function placeableStudent(orgId: string, schoolYearId: string, tag: string) {
    const account = await db.account.create({
      data: {
        org_id: orgId,
        role: 'student',
  ...
  it('(a) PROOF: the sweep can lock an application WHILE the approval is inside its window, and the approval write still lands (no write-time pending guard)', async () => {
    return check('item6 (a) approval + sweep both act on the same application', async () => {
      ...
      const preCheck = await registrarRepo.findApplicationDetail(orgA.id, appRow.id);
      ...
      const approvalWrite = db.$transaction(async (tx) => {
        ...
        markRead();
        await writeGate;
        const res = await approvalRepo.approveInTx(tx, appRow.id, actorId, 'race-account-id');
        return res.status;
      });
      ...
      log(`(a) final: ${finalState?.status}, resulting_account_id=${finalState?.resulting_account_id}`);
  ...
  it('(b) PROOF: two section reads can overlap the same snapshot and BOTH land, oversubscribing past capacity', async () => {
    return check('item6 (b) count-snapshot race oversubscribes the section', async () => {
      ...
      for (let i = 0; i < 4; i++) {
        await existingEnrollment(orgB.id, sy.id, program.id, level.id, section.id, i);
      }
      ...
      await Promise.race([
        ...
        new Promise((_res, rej) =>
          setTimeout(() => rej(new Error(`read barrier timeout: reads=${reads}`)), 30000),
        ),
      ]);
      release(); // both txns continue their unconditional writes simultaneously
      const seenByRacer = await Promise.all([pA, pB]);
      log(`(b) both racers read the SAME snapshot count=${seenByRacer[0]}/${seenByRacer[1]}`);
      expect(seenByRacer[0]).toBe(4);
      expect(seenByRacer[1]).toBe(4);

      const afterCount = await activeCountInSection(section.id);
      const sec = await db.section.findUnique({ where: { id: section.id } });
      log(`(b) section after overlapping approvals: ${afterCount}/${sec?.capacity}`);
  ...
  afterAll(async () => {
    ...
    for (const org of [orgA, orgB]) {
      ...
      await step(`org ${org.id} sections`, () => db.section.deleteMany({ where: { org_id: org.id } }));
      await step(`org ${org.id} levels`, () => db.level.deleteMany({ where: { org_id: org.id } }));
      await step(`org ${org.id} programs`, () => db.program.deleteMany({ where: { org_id: org.id } }));
      await step(`org ${org.id} school years`, () =>
        db.schoolYear.deleteMany({ where: { org_id: org.id } }));
      await step(`org ${org.id}`, () => db.organization.deleteMany({ where: { id: org.id } }));
    }
    await app.close();
  }, 120000);
});