...
runSuite('Lane 1 item 8 — cross-school-year semester template (real DB)', () => {
  let app: INestApplication;
  let db: DatabaseService;
  let templateService: SemesterTemplateService;
  let templateRepo: SemesterTemplateRepository;

  const org = { id: `e2e8-org-${uuid()}`, slug: `e2e8-${uuid().slice(0, 8)}` };
  ...
  beforeAll(async () => {
    ...
    await db.organization.create({ data: { id: org.id, name: 'E2E Item 8 Org', slug: org.slug } });
    ...
    programA = await db.program.create({
      data: { org_id: org.id, school_year_id: syA.id, name: 'Program A (SYA)', type: 'elementary' },
    });
    programB = await db.program.create({
      data: { org_id: org.id, school_year_id: syB.id, name: 'Program B (SYB)', type: 'elementary' },
    });
  }, 120000);

  it('(a) one template assigned to two programs keeps each assignment’s term dates isolated', async () => {
    return check('item8 (a) assignment + per-assignment term-date isolation', async () => {
      ...
      log(`(a) template ${template.id} semesters=${template.semesters.length}`);

      // Assign the SAME template to programA (SY A) and programB (SY B).
      await templateRepo.assignToProgram({ orgId: org.id, programId: programA.id, templateId: template.id });
      await templateRepo.assignToProgram({ orgId: org.id, programId: programB.id, templateId: template.id });

      const assA = await templateRepo.findAssignmentByProgram(programA.id, org.id);
      const assB = await templateRepo.findAssignmentByProgram(programB.id, org.id);
      ...
      expect(localDate(datesA[0].start_date)).not.toBe(localDate(datesB[0].start_date));
    });
  }, 120000);

  it('(b) same-named component rows are scoped per school year — no cross-SY leak', async () => {
    return check('item8 (b) school-year isolation of component rows', async () => {
      ...
      log(
        `(b) "1st Semester" rows: ${firstSemesters.length} -> ` +
          firstSemesters
            .map((s) => `${s.school_year_id.slice(0, 6)}=${localDate(s.start_date)}..${localDate(s.end_date)}`)
            .join(' | '),
      );
      ...
      expect(localDate(semA!.start_date)).not.toBe(localDate(semB!.start_date));
      ...
      const allSemesters = await db.semester.findMany({ where: { org_id: org.id }, select: { school_year_id: true } });
  ...
  afterAll(async () => {
    ...
    await step('terms', () => db.term.deleteMany({ where: { org_id: org.id } }));
    await step('semesters', () => db.semester.deleteMany({ where: { org_id: org.id } }));
    await step('template terms', () => db.semesterTemplateTerm.deleteMany({ where: { org_id: org.id } }));
    await step('template items', () => db.semesterTemplateItem.deleteMany({ where: { org_id: org.id } }));
    await step('templates', () => db.semesterTemplate.deleteMany({ where: { org_id: org.id } }));
