...
export class GradeLockValidator {
  ...
  async validateReadiness(classId: string, orgId: string): Promise<ReadinessValidationResult> {
    ...
    const nameMap = enrolledIds.length > 0
    ...
    const nonOptionalComponents = scheme?.components?.filter((c: any) => !c.is_optional) ?? []
    ...
    for (const term of terms) {
      const assessments = await this.gradeRepo.findAssessmentsForTerm(classId, term.id, orgId)
      ...
      const submissions = await this.gradeRepo.findSubmissionsForTerm(classId, term.id, orgId)
