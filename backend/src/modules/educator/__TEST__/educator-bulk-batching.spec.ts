...
describe('EducatorService.bulkCreate — batched org lookup', () => {
  ...
  it('fetches org once for N entries and keeps created/skipped output', async () => {
    ...
    expect(res.created[0].email).toBe('johndoe@school.educator.edu');
    // Third entry duplicates the first email name → suffixed, still created.
    expect(res.created[2].email).toContain('johndoe1@school.educator.edu');
  });

  it('still skips database and in-file duplicates', async () => {
    const { service, repo } = makeService();
    repo.findEmailsInBatch.mockResolvedValue(['johndoe@school.educator.edu']);
    const res = (await service.bulkCreate('org-1', [
      { fullName: 'John Doe', id: 'EDU-1' },
    ...
    expect(res.skipped).toEqual([
      expect.objectContaining({ email: 'johndoe@school.educator.edu', reason: 'duplicate_in_database' }),
    ]);
    expect(res.created[0].email).toContain('johndoe1@school.educator.edu');
  });
});
