...
export class StudentEnrollmentService {
  ...
  async getEnrolledStudents(
    ...
    const [data, total] = await this.repo.findAllBySchoolYear(
      schoolYearId,
      orgId,
      page,
      limit,
  ...
  async enrollStudent(
    ...
    const activeElsewhere = await this.repo.findActiveEnrollmentForStudent(
      dto.student_id,
      orgId,
    );
    if (activeElsewhere) {
  ...
  async updateEnrollment(
    ...
    const record = await this.repo.findEnrollmentById(enrollmentId, orgId);
    if (!record) throw new NotFoundException('Enrollment not found.');
    const result = await this.repo.updateEnrollmentStatus(
      enrollmentId,
      dto.status,
      dto.notes,
    );
  ...
  async enrollInProgram(
    ...
    if (!schoolYearEnrollment) {
      throw new BadRequestException(
        'Student is not enrolled in this school year.',
      );
    }
    if (schoolYearEnrollment.status !== SchoolYearEnrollmentStatus.active) {
      throw new BadRequestException(
        'Student school year enrollment is not active.',
      );
    }
    ...
    const programEnrollment = await this.repo.enrollInProgram(
      orgId,
      schoolYearEnrollment.id,
      dto,
      tx,
    );
  ...
  async updateProgramEnrollment(
    ...
    if (dto.section_id !== undefined) {
      ...
      if (dto.section_id !== null && dto.section_id !== fromSectionId) {
        const section = await this.sectionService.findById(
          dto.section_id,
          orgId,
        );

        if (section.level_id !== record.level_id) {
          throw new BadRequestException(
            'Cannot assign a student to a section from a different level.',
          );
        }
        if (record.course_id && section.course_id !== record.course_id) {
          throw new BadRequestException(
            'Cannot assign a student to a section from a different course.',
          );
        }
        if (record.strand_id && section.strand_id !== record.strand_id) {
          throw new BadRequestException(
            'Cannot assign a student to a section from a different strand.',
          );
        }
        ...
        if (enrolled >= section.capacity) {
          throw new ConflictException(
            `Section "${section.name}" is full (${enrolled}/${section.capacity}).`,
          );
        }
      }

      const result = await this.repo.updateProgramEnrollment(
        programEnrollmentId,
        {
      ...
      this.auditLogService
        .logAdminAction({
          ...
          metadata: {
            fromSectionId,
            toSectionId: dto.section_id,
            studentId: record.studentSchoolYear?.student_id ?? null,
          },
        })
        .catch(() => {});
