...
import {
  ...
  QueryAssessmentDto, PublishScoresDto, GradeEssayDto,
  UpdateSubmissionStatusDto, AssignStudentsDto, ReopenAssessmentDto,
  SetGradeVisibilityDto, GradingMode,
...
export class AssessmentEducatorService {
  ...
  async create(classId: string, orgId: string, educatorId: string, dto: CreateAssessmentDto) {
    ...
    const assessment = await this.creation.createAssessmentRecord(orgId, classId, dto, effectiveGradingMode);
    ...
    if (isManual) {
      ...
      if (aiRanges.length > 0) {
        await this.generation.startGeneration(assessment.id, orgId, educatorId, { ...dto, ranges: aiRanges });
      }
    }

    await this.auditLog.logActivityEvent({ orgId, actorId: educatorId, action: 'assessment_created', entityType: 'class', entityId: classId, metadata: { assessmentId: assessment.id, type: dto.type, gradingMode: effectiveGradingMode } });
    return assessment;
  }
  ...
  async findAll(classId: string, orgId: string, educatorId: string, query: QueryAssessmentDto) {
    ...
    const [submissionCounts, submittedSubs, essayQuestions, activeReopens] = await Promise.all([
      this.db.submission.groupBy({ by: ['assessment_id', 'status'], where: { assessment_id: { in: ids } }, _count: { id: true } }),
      this.db.submission.findMany({ where: { assessment_id: { in: ids }, status: 'submitted' }, select: { id: true, assessment_id: true } }),
      this.db.question.findMany({ where: { assessment_id: { in: ids }, type: 'essay' }, select: { id: true, assessment_id: true } }),
      this.db.submission.findMany({ where: { assessment_id: { in: ids }, reopened_until: { gt: new Date() } }, select: { assessment_id: true, reopened_until: true }, orderBy: { reopened_until: 'desc' } }),
    ]);
    ...
    const pendingEssaySubIds = submittedSubs.length && essayQIds.length
      ? await this.db.submissionAnswer.groupBy({ by: ['submission_id'], where: { submission_id: { in: submittedSubs.map((s) => s.id) }, question_id: { in: essayQIds }, is_correct: null }, _count: { id: true } })
      : [];
    ...
    for (const r of pendingEssaySubIds) {
      ...
      if (assId) pendingByAssessment.set(assId, (pendingByAssessment.get(assId) ?? 0) + 1);
    }

    const submittedMap = new Map<string, number>();
    for (const s of submissionCounts) {
      if (s.status === 'submitted') submittedMap.set(s.assessment_id, s._count.id);
    }

    const reopenMap = new Map<string, Date>();
    for (const r of activeReopens) {
      if (!reopenMap.has(r.assessment_id)) reopenMap.set(r.assessment_id, r.reopened_until!);
    }
  ...
  async findOne(id: string, orgId: string, educatorId: string) {
    ...
    const [questions, activeReopen] = await Promise.all([
      ...
      this.db.submission.findFirst({ where: { assessment_id: id, reopened_until: { gt: new Date() } }, select: { reopened_until: true }, orderBy: { reopened_until: 'desc' } }),
    ]);
    return { ...assessment, questions, generationStatus: this.generation.getGenerationStatus(id)?.status ?? 'completed', reopenedUntil: activeReopen?.reopened_until ?? null };
  }
  ...
  async update(id: string, orgId: string, educatorId: string, dto: UpdateAssessmentDto) {
    const assessment = await this.core.findAssessmentOrThrow(id, orgId);
    await this.assertEducatorOwnsClass(assessment.class_id, orgId, educatorId);
    const updated = await this.repo.update(id, { releaseDate: dto.releaseDate ? new Date(dto.releaseDate) : undefined, endDate: dto.endDate ? new Date(dto.endDate) : undefined, type: dto.type, showBreakdown: dto.showBreakdown, gradingMode: dto.gradingMode, manualMaxScore: dto.manualMaxScore, weekNumber: dto.weekNumber });
    await this.auditLog.logActivityEvent({ orgId, actorId: educatorId, action: 'assessment_edited', entityType: 'class', entityId: assessment.class_id, metadata: { assessmentId: id } });
    return updated;
  }
  ...
  async delete(id: string, orgId: string, educatorId: string) {
    ...
    await this.auditLog.logActivityEvent({ orgId, actorId: educatorId, action: 'assessment_deleted', entityType: 'class', entityId: assessment.class_id, metadata: { assessmentId: id } });
    if (assessment.term_id) {
      this.gradeService.computeGrades(assessment.class_id, assessment.term_id, orgId, educatorId)
        .catch((err: Error) => this.logger.error(`[Grade] Recompute failed after delete ${id}: ${err.message}`));
    }
    return { success: true };
  }
  ...
  async updateQuestion(assessmentId: string, questionId: string, orgId: string, educatorId: string, dto: UpdateQuestionDto) {
    ...
    return this.repo.updateQuestion(questionId, { questionText: dto.questionText, correctAnswer: dto.correctAnswer });
  }
  ...
  async updateSubmissionStatus(assessmentId: string, submissionId: string, orgId: string, educatorId: string, dto: UpdateSubmissionStatusDto) {
    const assessment = await this.core.findAssessmentOrThrow(assessmentId, orgId);
    await this.assertEducatorOwnsClass(assessment.class_id, orgId, educatorId);
    const updated = await this.submission.updateSubmissionStatus(assessment, submissionId, orgId, dto);
    if (assessment.term_id) {
      this.gradeService.computeGrades(assessment.class_id, assessment.term_id, orgId, educatorId)
        .catch((err: Error) => this.logger.error(`[Grade] Recompute failed after status change: ${err.message}`));
    }
    return updated;
  }
  ...
  async assignStudents(assessmentId: string, orgId: string, educatorId: string, dto: AssignStudentsDto) {
    ...
    await this.auditLog.logActivityEvent({ orgId, actorId: educatorId, action: 'students_assigned_to_assessment', entityType: 'assessment', entityId: assessmentId, metadata: { assessmentId, studentIds: dto.studentIds } });
    return result;
  }
  ...
  async reopen(assessmentId: string, orgId: string, educatorId: string, dto: ReopenAssessmentDto) {
    ...
    await this.auditLog.logActivityEvent({ orgId, actorId: educatorId, action: 'assessment_reopened', entityType: 'assessment', entityId: assessmentId, metadata: { assessmentId, studentIds: dto.studentIds, reopenedUntil: dto.reopenedUntil } });
    return result;
  }
  ...
  async publishScores(assessmentId: string, orgId: string, educatorId: string, dto: PublishScoresDto) {
    ...
    await this.auditLog.logActivityEvent({ orgId, actorId: educatorId, action: 'score_published', entityType: 'class', entityId: assessment.class_id, metadata: { assessmentId, studentIds: dto.studentIds ?? 'all' } });
    await this.gradeService.registerAssessmentForAllStudents(assessmentId, assessment.class_id, orgId);
    if (assessment.term_id) {
      this.gradeService.computeGrades(assessment.class_id, assessment.term_id, orgId, educatorId)
        .catch((err: Error) => this.logger.error(`[Grade] Recompute failed after publish: ${err.message}`));
    }
    return { success: true };
  }
  ...
  async unpublishScores(assessmentId: string, orgId: string, educatorId: string) {
    ...
    await this.auditLog.logActivityEvent({ orgId, actorId: educatorId, action: 'score_unpublished', entityType: 'class', entityId: assessment.class_id, metadata: { assessmentId } });
    if (assessment.term_id) {
      this.gradeService.computeGrades(assessment.class_id, assessment.term_id, orgId, educatorId)
        .catch((err: Error) => this.logger.error(`[Grade] Recompute failed after unpublish: ${err.message}`));
    }
    return { success: true };
  }
  ...
  async generatePreview(classId: string, orgId: string, educatorId: string, dto: CreateAssessmentDto) {
    await this.assertEducatorOwnsClass(classId, orgId, educatorId);
    await this.creation.assertTypeMatchesScheme(classId, orgId, dto.type);
    const previewId = await this.generation.startPreview(classId, orgId, educatorId, dto);
    return { previewId };
  }
  ...
  async confirmPreview(previewId: string, classId: string, orgId: string, educatorId: string) {
    const result = this.generation.getPreviewResult(previewId);
    if (!result) throw new NotFoundException('Preview not found or expired. Please generate again.');
    const { questions: generated, dto } = result;
    if (!generated?.length) throw new NotFoundException('No questions in preview.');
    if (!dto.lessonId) throw new NotFoundException('lessonId is required.');

    const assessment = await this.creation.createAssessmentRecord(orgId, classId, dto, dto.gradingMode ?? GradingMode.SYSTEM);
    await this.repo.createQuestions(generated.map((q, idx) => ({
      orgId, assessmentId: assessment.id,
      type: q.type,
    ...
    this.logger.log(`[Assessment] ${generated.length} questions confirmed for ${assessment.id}`);
  ...
  async onSubmissionFinished(data: { orgId: string; classId: string; studentId: string; submittedAt: Date }) {
    this.attendanceService.markPresentFromSubmission(data).catch((err) => {
      this.logger.error(`[AttendanceService] Failed to auto-mark present for student ${data.studentId}: ${err}`);
    });
  }
}