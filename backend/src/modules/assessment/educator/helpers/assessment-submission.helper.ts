...
import { UpdateSubmissionStatusDto, AssignStudentsDto, ReopenAssessmentDto, GradeEssayDto } from '../../dto/assessment.dto';
...
export class AssessmentSubmissionHelper {
  ...
  async getSubmissions(assessment: any, orgId: string) {
    ...
    const profiles = await this.db.profile.findMany({
      ...
      select: { account_id: true, full_name: true, account: { select: { email: true } } },
    });
    ...
    return studentIds.map((studentId) => {
      ...
      return {
        ...
        status: !sub ? 'not_started' : (sub.status as string),
        score: sub?.score ?? null,
        manual_score: sub?.manual_score ?? null,
        total_points: assessment.total_items,
        is_published: assessment.is_published,
        essay_graded: false,
        answers: subAnswers.map((a) => ({ id: a.id, questionId: a.question_id, answer: a.answer, isCorrect: a.is_correct })),
        started_at: null,
  ...
  async updateSubmissionStatus(assessment: any, submissionId: string, orgId: string, dto: UpdateSubmissionStatusDto) {
    ...
    if (!submission || submission.assessment_id !== assessment.id) {
      ...
      const studentId = submissionId.startsWith(prefix) ? submissionId.slice(prefix.length) : null;
      ...
      submission = await this.db.submission.create({
        data: { org_id: orgId, assessment_id: assessment.id, student_id: studentId, status: 'draft' },
      });
    }
    ...
    if (dto.status === 'exempted') { updateData.isExempted = true; updateData.score = 0; }
    if (dto.status === 'missed') { updateData.status = 'custom'; updateData.isMissed = true; updateData.score = 0; }
  ...
  async gradeEssay(assessment: any, submissionId: string, dto: GradeEssayDto) {
    ...
    if (!submission || submission.assessment_id !== assessment.id) throw new NotFoundException('Submission not found.');
  ...
  async assignStudents(assessment: any, orgId: string, dto: AssignStudentsDto) {
    for (const studentId of dto.studentIds) {
      await this.repo.upsertSubmission({ orgId, assessmentId: assessment.id, studentId, status: 'draft' }).catch(() => {});
    }
  ...
  async reopen(assessment: any, orgId: string, dto: ReopenAssessmentDto) {
    ...
    for (const studentId of dto.studentIds) {
      const existing = await this.repo.findSubmissionByStudent(assessment.id, studentId);
      if (existing) {
        await this.db.submissionAnswer.deleteMany({ where: { submission_id: existing.id } });
        await this.db.submission.update({
          where: { id: existing.id },
          data: { status: 'draft', score: null, manual_score: null, submitted_at: null, reopened_until: reopenedUntil },
        });
      } else {
        await this.db.submission.create({
          data: { org_id: orgId, assessment_id: assessment.id, student_id: studentId, status: 'draft', reopened_until: reopenedUntil },
        });
      }
    }
