...
describe('GradeCoreService.computeWeightedScore', () => {
  it('happy path: sums weighted category averages into the final score (maxScore categories)', () => {
    ...
    expect(
      service.computeWeightedScore(submissions, [], allAssessments, categories),
    ).toBe(76);
  });
  ...
  it('KNOWN BUG R1: a fully-exempted maxScore category must be excluded from the average, not zero-penalized at full weight', () => {
    ...
    const allAssessments = [
      assessment('a1', 'quiz', 50),
      assessment('a2', 'performance', 100),
    ];
  ...
  it('KNOWN BUG R2: a missing submission must be treated identically with and without maxScore (counted as 0, never ignored)', () => {
    const assessments = [
      assessment('a1', 'quiz', 100),
      assessment('a2', 'quiz', 20),
    ];
    const submissions = [completed('a1', 100)]; // a2 has NO submission at all

    const withoutMaxScore: Category[] = [
      { name: 'Quiz', type: 'quiz', weight: 100 },
    ];
    const withMaxScore: Category[] = [
      { name: 'Quiz', type: 'quiz', weight: 100, maxScore: 120 },
    ];

    // Non-maxScore branch counts the missing item as 0: (100 + 0)/2 = 50.
    expect(
      service.computeWeightedScore(
        submissions,
        [],
        assessments,
        withoutMaxScore,
    ...
    expect(
      service.computeWeightedScore(submissions, [], assessments, withMaxScore),
    ).toBe(50);
  });
  ...
  it('KNOWN BUG R3: a hybrid score that exceeds the assessment total must never push the final score past 100', () => {
    const categories: Category[] = [
      { name: 'Exam', type: 'exam', weight: 100 },
    ];
  ...
  it.each([
    ...
    [
      [{ name: 'Quiz', type: 'quiz', weight: 100, maxScore: 50 }],
      'no assessments of a matched type',
    ],
  ] as [Category[], string][])(
    'invalid/empty input: returns 0 when %s',
    (categories) => {
  ...
  it('partial exemption: exempted assessments are excluded from the average — same result with and without maxScore', () => {
    const allAssessments = [
      assessment('a1', 'quiz', 10),
      assessment('a2', 'quiz', 10),
    ];
    ...
    const withMaxScore: Category[] = [
      { name: 'Quiz', type: 'quiz', weight: 100, maxScore: 100 },
    ];
    expect(
      service.computeWeightedScore(submissions, [], allAssessments, plain),
    ).toBe(80);
    expect(
      service.computeWeightedScore(
        submissions,
  ...
  it('regression lock for R3: capped categories cannot be recombined to exceed 100', () => {
    ...
    const allAssessments = [
      assessment('a1', 'quiz', 20, 'hybrid'),
      assessment('a2', 'exam', 20, 'hybrid'),
    ];
...
describe('GradeCoreService.mergeHybridScores', () => {
  it.each([
    ...
    [
      { manual_score: null, score: 20 },
      20,
      'falls back to score when manual_score is null',
    ],
    [
      { manual_score: undefined, score: undefined },
      0,
      'falls back to 0 when both are absent',
    ],
  ] as [Submission, number, string][])(
    'returns exact raw sum/fallback: %s',
    (submission, expected) => {
...
describe('GradeCoreService.resolveGrade', () => {
  ...
  it.each([
    ...
    'maps in-range and exact-boundary score %i to %s',
    (score, expected) => {
  ...
  it.each([
    ...
    'round-half-up fractional boundary %f resolves to %s (never N/A inside the scale)',
    (score, expected) => {
      expect(service.resolveGrade(score, RANGES)).toBe(expected);
    },
  );
  ...
  it.each([[-1], [50], [95], [200]] as [number][])(
    'returns N/A for out-of-range score %i',
    (score) => {
