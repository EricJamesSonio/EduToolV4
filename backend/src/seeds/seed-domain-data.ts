...
import { allMajorSubjects, allMinorSubjects, deriveProgramKey } from '../modules/org-seeder/data/subjects'
...
const ADMIN_CONFIGS: AdminSeedConfig[] = [
  { adminIndex: 0, programs: ['daycare', 'kinder', 'elementary', 'jhs', 'shs', 'college'], educators: 30, students: 500 },
  ...
  { adminIndex: 7, programs: ['kinder', 'daycare'], educators: 20, students: 150 },
]
...
function buildEducatorEmail(orgEmailExtension: string, localPart: string): string {
...
function buildStudentEmail(orgEmailExtension: string, localPart: string): string {
  const base = orgEmailExtension
    .replace(/^@/, '')
    .replace(/\.(student|educator)\./g, '.')
    .trim()
...
async function seedPrograms(orgId: string, schoolYearId: string, programKeys: string[]): Promise<Record<string, string>> {
  ...
  for (const p of PROGRAMS) {
    ...
    if (existing) {
      ...
      const rec = await db.program.create({
        data: { id, org_id: orgId, school_year_id: schoolYearId, name: p.name, type: p.type },
      })
...
async function seedCourses(orgId: string, schoolYearId: string, programMap: Record<string, string>): Promise<Record<string, string>> {
  ...
  for (const c of [...COLLEGE_COURSES, ...BSED_MAJORS]) {
    ...
    if (existing) {
      ...
      const rec = await db.course.create({
        data: { id, org_id: orgId, school_year_id: schoolYearId, program_id: collegeId, name: c.name, code: c.code },
      })
...
async function seedStrands(orgId: string, schoolYearId: string, programMap: Record<string, string>): Promise<Record<string, string>> {
  ...
  for (const s of SHS_STRANDS) {
    ...
    if (existing) {
      ...
      const rec = await db.strand.create({
        data: { id, org_id: orgId, school_year_id: schoolYearId, program_id: shsId, name: s },
      })
      strandMap[s] = rec.id
    }
  }
...
async function seedLevelsAndSections(
  ...
  const allDefs = buildLevelDefs().filter((l) => programKeys.includes(l.programKey))
  ...
  for (const def of allDefs) {
    ...
    if (progKey === 'college' && def.courseCode) {
      ...
      levelId = seedId('level', progKey, def.courseCode, def.name, schoolYearId, orgId)
    ...
    if (existing) {
      ...
      const rec = await db.level.create({
        data: {
          ...
          ...(progKey === 'college' && def.courseCode ? { course_id: courseMap[def.courseCode] ?? undefined } : {}),
          name: def.name,
        },
      })
    ...
    const sections = def.sections ?? [{ name: 'Section A', capacity: 40 }, { name: 'Section B', capacity: 40 }]
    for (const sec of sections) {
      const sectionId = seedId('section', progKey, levelKey, sec.name, schoolYearId, orgId)
      const existingSec = await db.section.findFirst({ where: { id: sectionId } })
      if (!existingSec) {
  ...
  if (programKeys.includes('shs') && programMap['shs']) {
    ...
    for (const [strandName, strandId] of Object.entries(strandMap)) {
      for (const def of shsDefs) {
        ...
        const levelId = seedId('level', 'shs', strandName, def.name, schoolYearId, orgId)
        const existing = await db.level.findFirst({ where: { id: levelId } })
        if (existing) {
          levelMap[levelKey] = existing.id
        } else {
        ...
        const sections = def.sections ?? [{ name: 'Section A', capacity: 40 }, { name: 'Section B', capacity: 40 }]
        for (const sec of sections) {
          const sectionId = seedId('section', 'shs', strandName, def.name, sec.name, schoolYearId, orgId)
          const existingSec = await db.section.findFirst({ where: { id: sectionId } })
          if (!existingSec) {
            await db.section.create({
              data: {
  ...
  if (programKeys.includes('college') && programMap['college']) {
    const collegeDefs = allDefs.filter((l) => l.programKey === 'college' && l.courseCode)
    for (const [courseCode, courseId] of Object.entries(courseMap)) {
      ...
      for (const def of courseDefs) {
        ...
        const levelId = seedId('level', 'college', courseCode, def.name, schoolYearId, orgId)
        const existing = await db.level.findFirst({ where: { id: levelId } })
        if (existing) {
          levelMap[levelKey] = existing.id
        } else {
        ...
        const sections = def.sections ?? [{ name: 'Section A', capacity: 50 }, { name: 'Section B', capacity: 50 }]
        for (const sec of sections) {
          const sectionId = seedId('section', 'college', courseCode, def.name, sec.name, schoolYearId, orgId)
          const existingSec = await db.section.findFirst({ where: { id: sectionId } })
          if (!existingSec) {
            await db.section.create({
              data: {
...
async function seedGradingScales(orgId: string, schoolYearId: string, programKeys: string[], programMap: Record<string, string>): Promise<void> {
  ...
  for (const { programKey, scaleName, ranges } of assignments) {
    ...
    const existingAssign = await db.gradingScaleAssignment.findFirst({ where: { id: assignmentId } })
...
async function seedGradingSchemes(orgId: string, programKeys: string[], programMap: Record<string, string>): Promise<void> {
  ...
  for (const preset of SCHEME_PRESETS) {
    ...
    const existing = await db.gradingSchemeTemplate.findFirst({ where: { id } })
    if (existing) continue

    const template = await db.gradingSchemeTemplate.create({
      data: { id, org_id: orgId, name: preset.name, program_type: progKey ?? null },
    })
...
async function seedSemesterTemplates(
  ...
  for (const tpl of SEMESTER_TEMPLATES) {
    ...
    const existing = await db.semesterTemplate.findFirst({ where: { id: templateId } })
    ...
    const template = await db.semesterTemplate.create({
      data: { id: templateId, org_id: orgId, program_type: tpl.programType, name: tpl.name },
    })
    ...
    const termDateData = computeTermDates(SY_START, SY_END, tpl, termIds).map((td) => ({
...
async function seedSubjects(
  ...
  // Major subjects
  const majorDefs = allMajorSubjects().filter((s) => programKeys.includes(deriveProgramKey(s.levelName)))
  ...
  if (programKeys.includes('college') && programMap['college']) {
    const collegeMinors = allMinorSubjects().filter((s) => deriveProgramKey(s.levelName) === 'college')
    const courseCodes = Object.keys(courseMap)
    const firstCourseCode = courseCodes[0]

    for (const s of collegeMinors) {
      const levelId = s.yearLevel && firstCourseCode ? levelMap[`${firstCourseCode}|${s.yearLevel}`] : null
      if (!levelId) continue
      ...
      for (const [code, cId] of Object.entries(courseMap)) {
        ...
        await db.subjectSharing.upsert({
          where: { id: sharingId },
          update: {},
          create: { id: sharingId, org_id: orgId, subject_id: subjectId, course_id: cId, strand_id: null, level_id: null },
        })
      }
    }
  }
...
async function seedEducators(orgId: string, emailExtension: string, count: number): Promise<string[]> {
...
async function seedStudents(
  ...
  const sectionsByProgram: Record<string, { sectionId: string; levelId: string; levelName: string }[]> = {}
  ...
  for (const sec of allSections) {
    ...
    if (progKey) {
      sectionsByProgram[progKey].push({ sectionId: sec.id, levelId: level.id, levelName: level.name })
    }
  }
  ...
  for (const progKey of programKeys) {
    const levels = await db.level.findMany({
      where: { org_id: orgId, school_year_id: schoolYearId, program_id: programMap[progKey] },
    })
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const ssyExisting = await db.studentSchoolYear.findFirst({ where: { id: ssyId } })
    if (!ssyExisting) {
...
async function seedClasses(
  ...
  for (const progKey of programKeys) {
    ...
    const firstSem = assignment.template.semesters.find((s) => s.order_index === 0)
    if (!firstSem) continue

    // Create a Semester record in the Semester table
    const semesterId = seedId('semester', progKey, 'Sem 1', schoolYearId, orgId)
    const existingSem = await db.semester.findFirst({ where: { id: semesterId } })
    if (!existingSem) {
      await db.semester.create({
        data: {
          ...
          name: firstSem.name,
          start_date: SY_START,
          end_date: new Date(SY_START.getTime() + (SY_END.getTime() - SY_START.getTime()) / 2),
        },
      })
    }
  ...
  for (const subj of allSubjects) {
    ...
    if (!progId) continue
    const progKey = Object.entries(programMap).find(([, id]) => id === progId)?.[0]
    if (!progKey) continue
  ...
  for (const progKey of programKeys) {
    ...
    for (let c = 0; c < classCount; c++) {
      ...
      const educator = pick(educatorIds)
      const section = allSections.length > 0 ? pick(allSections) : null

      const classId = seedId('class', progKey, subject.id, String(c), schoolYearId, orgId)
      const existingClass = await db.class.findFirst({ where: { id: classId } })
            ...
            create: [
              {
                ...
                org_id: orgId,
                weekday: randInt(0, 4),
                start_time: new Date('2025-07-01T08:00:00Z'),
                end_time: new Date('2025-07-01T09:00:00Z'),
              },
            ],
          },
        },
...
async function main() {
  ...
  await db.account.upsert({
    ...
    update: { password: platformOwnerPw, role: 'platform_owner', status: AccountStatus.active, deleted_at: null },
    create: { email: platformOwnerEmail, password: platformOwnerPw, role: 'platform_owner', status: AccountStatus.active },
  })
  await db.profile.upsert({
    where: { account_id: (await db.account.findUnique({ where: { email: platformOwnerEmail } }))!.id },
    update: { full_name: 'Platform Owner' },
    create: { account_id: (await db.account.findUnique({ where: { email: platformOwnerEmail } }))!.id, full_name: 'Platform Owner' },
  })
  console.log(`  platform_owner  ${platformOwnerEmail}`)

  for (let i = 0; i < ADMIN_CONFIGS.length; i++) {
    const { adminIndex, programs: progKeys, educators: educatorCount, students: studentCount } = ADMIN_CONFIGS[i]
    ...
    const org = await db.organization.upsert({
      ...
      update: { name: school.name, address: school.address, logo_url: school.logo_url },
      create: { name: school.name, address: school.address, logo_url: school.logo_url, email_extension: emailExt },
    })

    const account = await db.account.upsert({
      where: { email: admin.email },
      update: { password: adminPw, role: 'admin', status: AccountStatus.active, deleted_at: null, org_id: org.id },
      create: { org_id: org.id, email: admin.email, password: adminPw, role: 'admin', status: AccountStatus.active },
    })
    await db.profile.upsert({
    ...
    if (!org.admin_account_id) {
      await db.organization.update({ where: { id: org.id }, data: { admin_account_id: account.id } })
    }
    ...
    await db.orgEnrollmentSetting.upsert({
      where: { org_id: org.id },
      update: {},
      create: { id: seedId('org-enrollment-setting', org.id), org_id: org.id, require_semester_reenrollment: false, auto_unenroll_on_year_end: true },
    })
    ...
    if (Object.keys(courseMap).length > 0) console.log(`  └ courses: ${Object.keys(courseMap).length}`)
    const strandMap = await seedStrands(org.id, schoolYearId, programMap)
    if (Object.keys(strandMap).length > 0) console.log(`  └ strands: ${Object.keys(strandMap).length}`)

    // e) Levels & sections
    const levelMap = await seedLevelsAndSections(org.id, schoolYearId, progKeys, programMap, courseMap, strandMap)
    ...
    // i) Subjects
    const subjectIds = await seedSubjects(org.id, schoolYearId, progKeys, programMap, courseMap, strandMap, levelMap)
    ...
    // k) Students
    const studentIds = await seedStudents(org.id, emailExt, studentCount, levelMap, progKeys, programMap, courseMap, strandMap, schoolYearId)
    console.log(`  └ students: ${studentIds.length}`)

    // l) Classes
    await seedClasses(org.id, schoolYearId, progKeys, programMap, subjectIds, educatorIds, levelMap)
